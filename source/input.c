...
int input_read_parameters(
 ...
 /* Omega_0_drf dark radiation fluid */
  class_call(parser_read_double(pfc,"N_drf",&param1,&flag1,errmsg),
             errmsg,
             errmsg);
  if (flag1 == _TRUE_) {
    pba->Omega0_drf = param1*7./8.*pow(4./11.,4./3.)*pba->Omega0_g;
  }

  Omega_tot += pba->Omega0_drf;

  /* Gamma0_dmdrf */
  class_call(parser_read_double(pfc,"Gamma0_dmdrf",&param1,&flag1,errmsg),
             errmsg,
             errmsg);
  if (flag1 == _TRUE_) {
...
int input_default_params(
  ...
  pba->K = 0.;
  pba->sgnK = 0;
  pba->Omega0_lambda = 1.-pba->Omega0_k-pba->Omega0_g-pba->Omega0_ur-pba->Omega0_drf-pba->Omega0_b-pba->Omega0_cdm-pba->Omega0_ncdm_tot-pba->Omega0_dcdmdr;
  pba->Omega0_fld = 0.;
  pba->a_today = 1.;
  pba->w0_fld=-1.;
  pba->wa_fld=0.;
