...
int background_functions(
  ...
  if (pba->has_drf == _TRUE_) {
    pvecback[pba->index_bg_rho_drf] = pba->Omega0_drf * pow(pba->H0,2) / pow(a_rel,4);
...
int background_init(
  ...
  if (pba->background_verbose > 0) {
    ...
    if (pba->N_ncdm > 0) {
      ...
      Neff = (pba->Omega0_ur+pba->Omega0_drf)/7.*8./pow(4./11.,4./3.)/pba->Omega0_g;
...
int background_indices(
  ...
  /* - index for dark radiation fluid */
  class_define_index(pba->index_bg_rho_drf,pba->has_drf,index_bg,1);

  /* - index for Gamma_dmdrf */
  class_define_index(pba->index_bg_Gamma_dmdrf,pba->has_drf,index_bg,1);
...
int background_output_titles(struct background * pba,
  ...
  class_store_columntitle(titles,"(.)rho_drf",pba->has_drf);     /** dark radiation fluid */
  class_store_columntitle(titles,"(.)Gamma_dmdrf",pba->has_drf); /** dm-drf coupling strength */
...
int background_output_data(
  ...
  for (index_tau=0; index_tau<pba->bt_size; index_tau++){
    ...
    class_store_double(dataptr,pvecback[pba->index_bg_rho_drf],pba->has_drf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_Gamma_dmdrf],pba->has_drf,storeidx);
