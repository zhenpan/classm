...
int perturb_indices_of_perturbs(
  ...
  ppt->has_source_delta_drf = _FALSE_;
  ...
  ppt->has_source_theta_drf = _FALSE_;
  ...
  for (index_md = 0; index_md < ppt->md_size; index_md++) {
    ...
    if (_scalars_) {
      ...
      if (ppt->has_density_transfers == _TRUE_) {
        ...
        if (pba->has_drf == _TRUE_)
          ppt->has_source_delta_drf = _TRUE_;
        if (pba->has_dcdm == _TRUE_)
      ...
      if (ppt->has_velocity_transfers == _TRUE_) {
        ...
        if (pba->has_drf == _TRUE_)
          ppt->has_source_theta_drf = _TRUE_;
        if (pba->has_dcdm == _TRUE_)
      ...
      class_define_index(ppt->index_tp_delta_drf,  ppt->has_source_delta_drf, index_type,1);
      ...
      class_define_index(ppt->index_tp_theta_drf,  ppt->has_source_theta_drf, index_type,1);
...
int perturb_prepare_output(struct background * pba,
  ...
  if (ppt->k_output_values_num > 0) {
    ...
    if (ppt->has_scalars == _TRUE_){
      ...
      class_store_columntitle(ppt->scalar_titles,"delta_drf",pba->has_drf);
      class_store_columntitle(ppt->scalar_titles,"theta_drf",pba->has_drf);
...
int perturb_vector_init(
  ...
  if (_scalars_) {
    ...
    class_define_index(ppv->index_pt_delta_drf,pba->has_drf,index_pt,1); /* drf density */
    class_define_index(ppv->index_pt_theta_drf,pba->has_drf,index_pt,1); /* drf velocity */
  ...
  /** - case of switching approximation while a wavenumber is being integrated */
    ...
    if (_scalars_) {
      ...
      if (pba->has_drf == _TRUE_) {
        ...
        ppv->y[ppv->index_pt_delta_drf] =
          ppw->pv->y[ppw->pv->index_pt_delta_drf];

          ppv->y[ppv->index_pt_theta_drf] =
            ppw->pv->y[ppw->pv->index_pt_theta_drf];
      }
...
int perturb_initial_conditions(struct precision * ppr,
  ...
  double delta_drf=0., theta_drf=0.;
  double delta_dr=0;
  double q,epsilon,k2;
  int index_q,n_ncdm,idx;
  double rho_r,rho_m,rho_nu,rho_m_over_rho_r;
  double fracnu,fracg,fracb,fraccdm,fracdrf,om;
  ...
  if (_scalars_) {
    ...
    if (pba->has_drf == _TRUE_) {
      rho_r += ppw->pvecback[pba->index_bg_rho_drf];
    }
    ...
    /* f_drf = Omega_drf(t_i) / Omega_r(t_i) */
    fracdrf = 1.-fracnu-fracg;
    ...
    */
    ...
    if ((ppt->has_ad == _TRUE_) && (index_ic == ppt->index_ic_ad)) {
      ...
      /* drf has same initial conditions to those of photon*/
      if (pba->has_drf == _TRUE_) {
        ppw->pv->y[ppw->pv->index_pt_delta_drf] = ppw->pv->y[ppw->pv->index_pt_delta_g]; /* drf density */
        ppw->pv->y[ppw->pv->index_pt_theta_drf] = ppw->pv->y[ppw->pv->index_pt_theta_g]; /* drf velocity */
      }
    ...
    if (ppt->gauge == newtonian) {
      ...
      */
      ...
      if (pba->has_drf == _TRUE_) {
	delta_drf = ppw->pv->y[ppw->pv->index_pt_delta_drf];
	theta_drf = ppw->pv->y[ppw->pv->index_pt_theta_drf];
      }
      else { 
	delta_drf = 0.;
        theta_drf = 0.;
      ...
      delta_tot = (fracg*ppw->pv->y[ppw->pv->index_pt_delta_g]+fracnu*delta_ur+ fracdrf*delta_drf
	+ rho_m_over_rho_r*(fracb*ppw->pv->y[ppw->pv->index_pt_delta_b]+fraccdm*delta_cdm))/(1.+rho_m_over_rho_r);

      velocity_tot = ((4./3.)*(fracg*ppw->pv->y[ppw->pv->index_pt_theta_g]+fracnu*theta_ur+fracdrf*theta_drf) 
	+ rho_m_over_rho_r*fracb*ppw->pv->y[ppw->pv->index_pt_theta_b])/(1.+rho_m_over_rho_r);
      ...
      if (pba->has_drf == _TRUE_) {
        ppw->pv->y[ppw->pv->index_pt_delta_drf] -= 4.*a_prime_over_a*alpha;
        ppw->pv->y[ppw->pv->index_pt_theta_drf] += k*k*alpha;
      }
...
int perturb_total_stress_energy(
  ...
  if (_scalars_) {
    ...
    /* drf contribution */
    if (pba->has_drf == _TRUE_) {
      ppw->delta_rho +=  ppw->pvecback[pba->index_bg_rho_drf]*y[ppw->pv->index_pt_delta_drf];
      ppw->rho_plus_p_theta += 4./3.*ppw->pvecback[pba->index_bg_rho_drf]*y[ppw->pv->index_pt_theta_drf];
      ppw->delta_p += 1./3.*ppw->pvecback[pba->index_bg_rho_drf]*y[ppw->pv->index_pt_delta_drf];
      rho_plus_p_tot +=  4./3. * ppw->pvecback[pba->index_bg_rho_drf];
    }
...
int perturb_sources(
  ...
  if (_scalars_) {
    ...
    /* delta_drf */
    if (ppt->has_source_delta_drf == _TRUE_) {
      _set_source_(ppt->index_tp_delta_drf) = y[ppw->pv->index_pt_delta_drf];
    }
    ...
    /* theta_drf */
    if (ppt->has_source_theta_drf == _TRUE_) {
      _set_source_(ppt->index_tp_theta_drf) = y[ppw->pv->index_pt_theta_drf];
    }
...
int perturb_print_variables(double tau,
  ...
  if (_scalars_) {
   ...
   if (pba->has_drf == _TRUE_) {
      ...
      delta_drf = y[ppw->pv->index_pt_delta_drf];
      theta_drf = y[ppw->pv->index_pt_theta_drf];
    }
    ...
    if (ppt->gauge == synchronous) {
      ...
      if (pba->has_drf == _TRUE_) {
        delta_drf -= 4. * pvecback[pba->index_bg_H]*pvecback[pba->index_bg_a]*alpha;
        theta_drf += k*k*alpha;
      }
    ...
    /* Dark radiation fluid */
    class_store_double(dataptr, delta_drf, pba->has_drf, storeidx);
    class_store_double(dataptr, theta_drf, pba->has_drf, storeidx);
    /* Non-cold Dark Matter */
    if ((pba->has_ncdm == _TRUE_) && ((ppt->has_density_transfers == _TRUE_) || (ppt->has_velocity_transfers == _TRUE_) || (ppt->has_source_delta_m == _TRUE_))) {
      for(n_ncdm=0; n_ncdm < pba->N_ncdm; n_ncdm++){
...
int perturb_derivs(double tau,
  ...
  if (_scalars_) {
    ...
    /* ONLY  newtonian gauge in presence of dm-drf coupling*/
    if (pba->has_drf == _TRUE_) {

        dy[pv->index_pt_delta_drf] = -4./3.*(y[pv->index_pt_theta_drf]+metric_continuity);   /* drf density */
        dy[pv->index_pt_theta_drf] =  k*k/4.*y[pv->index_pt_delta_drf]+metric_euler;       /* drf velocity */

	if (pba->Gamma0_dmdrf !=0.) {
	  R_cdm = 0.75 * pvecback[pba->index_bg_rho_cdm]/pvecback[pba->index_bg_rho_drf];
	  dy[pv->index_pt_theta_cdm] += a*pvecback[pba->index_bg_Gamma_dmdrf]*(y[pv->index_pt_theta_drf] - y[pv->index_pt_theta_cdm]);
	  dy[pv->index_pt_theta_drf] += R_cdm*a*pvecback[pba->index_bg_Gamma_dmdrf]*(y[pv->index_pt_theta_cdm] - y[pv->index_pt_theta_drf]);  
        }
    }
